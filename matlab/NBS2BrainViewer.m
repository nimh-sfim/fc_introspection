...
for atlas = atlases
    for contrast = contrasts
        for elt = elts
            ...
            mat_file  = fullfile(work_path, "NBS_"+elt+"_s0.05_"+contrast+".mat");
            txt_file  = fullfile(work_path,"NBS_"+elt+"_s0.05_"+contrast+".txt");
            edge_file = fullfile(work_path,"NBS_"+elt+"_s0.05_"+contrast+".edge");
            if exist(mat_file, 'file') == 2
                ...
                switch num_networks
                    case 0
                        disp("++ INFO: ["+elt+","+atlas+","+contrast+"] --> No Significant connections.")
                    case 1
                        data = full(cell2mat(data.nbs.NBS.con_mat));
                        data = data + data.';
                        writematrix(data,txt_file,"Delimiter"," ");
                        movefile(txt_file,edge_file);
                        disp("++ INFO: ["+elt+","+atlas+","+contrast+"]  --> Number of Significant Connections is " + (sum(sum(data))/2) + " originated in " + sum(sum(data)>0) + " nodes.")
                    otherwise
