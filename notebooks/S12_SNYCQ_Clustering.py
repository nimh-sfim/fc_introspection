...
model = NMF_model(P, np.ones_like(P), dimension=DIM, method='admm', Wbound=(True, 1.0), sparsity_parameter=SPARSITY)

W, Q = model.decomposition()

W      = pd.DataFrame(W, index=SNYCQ.index, columns=['Factor {d}'.format(d=d+1) for d in range(DIM)])
Q      = pd.DataFrame(Q, index=SNYCQ.columns, columns=['Factor {d}'.format(d=d+1) for d in range(DIM)])
