...
# This notebook helps obtain a basic understanding of the components of the MPI dataset that will be used in this project. Here we are only interested in the NC subset becuase it contains the resting-state scans that were followed by the short NYCQ.
# 
...
# * ```NC_withSNYCQ_subjects.txt```: text file with the list of subjects that have at least one resting-state scan with SNYCQ answers. These deserve attempting pre-processing of their anatomical data.
...
from utils.basics import ORIG_SNYCQ_PATH, SBJS_WITH_SNYCQ_PATH,ANAT_PATHINFO_PATH, RESOURCES_SNYCQ_DIR, RESOURCES_DINFO_DIR
...
print('++ INFO [Output folder]: Folder containing all pre-processed versions of the SNYCQ:      %s' % RESOURCES_SNYCQ_DIR)
...
print("++ INFO [Input file]: Path to SNYCQ responses: %s" % snycq_data_path)
print("++ INFO [Input file]: Path to SNYCQ metadata: %s" % snycq_json_path)
...
for subject in subjectIDs:
    for run in rest_scanIDs:
        for question in rest_questionIDs:
            snycq_data.loc[(subject,run),question] = snycq['SNYCQ_'+run+'_'+question][subject]
...
if not osp.exists(RESOURCES_SNYCQ_DIR):
    os.makedirs(RESOURCES_SNYCQ_DIR)
    print('++ INFO: Created folder for SNYCQ resources: %s' % RESOURCES_SNYCQ_DIR)
...
if not osp.exists(RESOURCES_DINFO_DIR):
    os.makedirs(RESOURCES_DINFO_DIR)
    print('++ INFO: Created folder for dataset info resources: %s' % RESOURCES_DINFO_DIR)
...
for sbj in subjects_to_analyze:
    ...
    if osp.exists(path_ses01):
        anat_loc_df.loc[sbj,'ses-01'] = True
        anat_loc_df.loc[sbj,'anat_path'] = path_ses01
    else:
        anat_loc_df.loc[sbj,'ses-01'] = False
    if osp.exists(path_ses02):
        anat_loc_df.loc[sbj,'ses-02'] = True
        anat_loc_df.loc[sbj,'anat_path'] = path_ses02
    else:
        anat_loc_df.loc[sbj,'ses-02'] = False
