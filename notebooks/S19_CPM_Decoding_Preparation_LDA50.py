...
for model in CPM_models.keys():
    ...
    top10_degree_values = a.sort_values(ascending=False).values[0:10]
    for i,dv in enumerate(top10_degree_values):
        ii = str(i).zfill(2)
        output_path = osp.join(RESOURCES_NIMARE_DIR,f'{file_suffix}_Degree_Top{ii}_ROI.nii')
        command = f'''module load afni; 
                  3drefit -space MNI {input_path}; \
...
for model in CPM_models.keys():
    ...
    command = f'''module load afni; \
                  ...
                  3dMean -sum -overwrite -prefix {file_suffix}_Degree.TOP10.nii {file_suffix}_Degree_Top??_ROI.nii; \
                  3dcalc -overwrite -a {file_suffix}_Degree.TOP10.nii -b {file_suffix}_Degree.nii -exp "a*b" -prefix {file_suffix}_Degree.TOP10.nii; \
    '''
    output  = subprocess.check_output(command, shell=True, stderr=subprocess.STDOUT)
    print(output.strip().decode())
