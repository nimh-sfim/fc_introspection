...
# ======================================================
dimension_list = [1, 2, 3, 4, 5, 6, 7, 8]
sparsity_list  = [0, 1e-4, 1e-3, 1e-2, 2e-2, 3e-2, 4e-2, 5e-2, 6e-2, 7e-2, 8e-2, 9e-2, 1e-1, 2e-1, 3e-1,4e-1,5e-1,6e-1,7e-1,8e-1,9e-1, 1.0, 2.0]
#sparsity_list =  [0, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1] #[0.001, 0.003, 0.005, 0.007, 0.009, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 1.5, 2.0]
...
logs_folder = osp.join(PRJ_DIR,'Logs.{username}'.format(username=username),'SNYCQ_Model_cv')
...
for dimension in dimension_list:
    for sparsity in sparsity_list:
        for t in range(repeat):
            for j in range(J):
                file.write("export VAR_D={d} VAR_S={s} VAR_R={r} VAR_J={j} VAR_FILE={datafile}; sh {PRJ_DIR}/code/fc_introspection/notebooks/mlt/SNYCQ_model_cv.sh\n"
